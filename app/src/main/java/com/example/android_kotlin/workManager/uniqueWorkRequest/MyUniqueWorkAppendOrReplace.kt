...
import com.example.android_kotlin.workManager.uniqueWorkRequest.UniqueWorkRequestFragment.Companion.WORK_TYPE_APPEND_OR_REPLACE
import com.example.android_kotlin.workManager.uniqueWorkRequest.UniqueWorkRequestFragment.Companion.WORK_TYPE_REPLACE

class MyUniqueWorkAppendOrReplace(var context : Context, var params : WorkerParameters) : Worker(context, params) {
    override fun doWork(): Result {
        var workNo = params.inputData.getInt(WORK_TYPE_APPEND_OR_REPLACE,-1)
        sendBroadCast(WORK_TYPE_APPEND_OR_REPLACE, "Work ${workNo} Started")
        Thread.sleep(1000)

        for(i in 1..10){
            sendBroadCast(WORK_TYPE_APPEND_OR_REPLACE, "Work ${workNo} progress ${i*10}% completed")
            Thread.sleep(1000)
        }

        sendBroadCast(WORK_TYPE_APPEND_OR_REPLACE, "Work ${workNo} Finished")
