...
import com.example.android_kotlin.workManager.uniqueWorkRequest.UniqueWorkRequestFragment.Companion.WORK_TYPE
import com.example.android_kotlin.workManager.uniqueWorkRequest.UniqueWorkRequestFragment.Companion.WORK_TYPE_REPLACE

class MyUniqueWorkReplace(var context : Context, var params : WorkerParameters) : Worker(context, params) {
    override fun doWork(): Result {
        var workNo = params.inputData.getInt(WORK_TYPE_REPLACE,-1)
        sendBroadCast(WORK_TYPE_REPLACE, "Work ${workNo} Started")
        Thread.sleep(1000)

        for(i in 1..10){
            sendBroadCast(WORK_TYPE_REPLACE, "Work ${workNo} progress ${i*10}% completed")
            Thread.sleep(1000)
        }

        sendBroadCast(WORK_TYPE_REPLACE, "Work ${workNo} Finished")
